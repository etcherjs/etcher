...
export const generateCoreFile = async () => {
    ...
    const componentFiles = await fs.promises.readdir(path.join(config.srcDir, "components"));
    ...
    for (const file of componentFiles) {
        if (file.endsWith(".html")) {
            ...
            const componentData = await fs.promises.readFile(path.join(config.srcDir, "components", file), "utf8");

            toWrite += `{const doc=\`${componentData}\`;transformComponent(doc,"${componentName}");}`;

            console.log(chalk.greenBright(`Transformed ${file}!`));
        } else {
            console.log(chalk.yellow(`Skipping ${file} as it is not an HTML file.`));
        }
    }
    ...
    await fs.promises.writeFile(path.join(config.outDir, "_chisel.js"), (await minify(toWrite)).code);
...
export const watch = async () => {
    ...
    fs.watch(path.join(config.srcDir, "components"), async () => {
        ...
        console.log("");
        console.log("---------------------------------------------");
        console.log(chalk.white("Detected change in components, regenerating chisel..."));
    ...
    fs.watch(path.join(config.srcDir, "pages"), async () => {
        ...
        console.log("");
        console.log("---------------------------------------------");
        console.log(chalk.white("Detected change in pages, regenerating pages..."));
        await migratePages();
        console.log("---------------------------------------------");
    });
};
